...
let vertexShader = `
    ...
    void main(){
      ...
      float noiseVal = snoise(gridPosition/detailLow + time*3.0);
      float noiseDetailHighVal = snoise(gridPosition/detailHigh + time*3.0);

      float noise = noiseVal*100.0  + noiseDetailHighVal*10.0;

      
      aColor += 0.5+ (noiseVal + noiseDetailHighVal )/2.0;

      
      vec4 modelViewPosition =  modelViewMatrix * vec4(transformed, 1.0);

      // Scale the Object
      modelViewPosition.x *= noise/100.0;
      modelViewPosition.y *= noise/100.0;
      modelViewPosition.z *= noise/100.0;
...
function init() {
  ...
  instancedGeometry.translate(-instanceCount / 1.5, -instanceCount / 1.5, -instanceCount / 1.5)
...
function animate(time) {
  ...
  instancedMesh.rotation.x = time * 5;
  instancedMesh.rotation.y = time * 5;
  instancedMesh.rotation.z = time * 5;
